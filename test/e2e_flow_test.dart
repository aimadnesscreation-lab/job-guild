...
void main() {
  ...
  group('Supabase Schema Validation', () {
    ...
    test('2. Categories table has bilingual seed data', () async {
      final response = await db.from('categories').select('id, name_en, name_ur').order('id');
      expect(response, isA<List>());
      ...
      print('✅ categories: ${response.length} bilingual entries seeded');
    });
    ...
    test('3. Worker profiles table structure', () async {
      final response = await db
          ...
          .limit(1);
      expect(response, isA<List>());
      print('✅ worker_profiles: table exists');
    });
    ...
    test('4. Worker categories join table', () async {
      final response = await db
          ...
          .limit(1);
      expect(response, isA<List>());
      print('✅ worker_categories: join table exists');
    });
    ...
    test('5. Jobs table with all fields', () async {
      ...
      print('✅ jobs: table exists with required columns');
    });
    ...
    test('10. Favorites table (newly added)', () async {
      final response = await db
          ...
          .limit(1);
      expect(response, isA<List>());
      print('✅ favorites: table exists with user_id, favorited_user_id, created_at');
    });
    ...
    test('11. Reports table (newly added)', () async {
      final response = await db
          ...
          .limit(1);
      expect(response, isA<List>());
      print('✅ reports: table exists with status tracking');
    });

    test('12. PostGIS extension is enabled', () async {
      final response = await db.rpc('get_nearby_jobs', params: {
        'lat': 31.5204,
        'lng': 74.3587,
        'radius_km': 50.0,
      });
      expect(response, isA<List>());
      print('✅ PostGIS: get_nearby_jobs RPC function works');
    });

    test('13. Worker nearby query RPC works', () async {
      final response = await db.rpc('get_nearby_workers', params: {
        'lat': 31.5204,
        'lng': 74.3587,
        'radius_km': 50.0,
      });
      expect(response, isA<List>());
      print('✅ PostGIS: get_nearby_workers RPC function works');
    });

    test('14. Auth endpoint is accessible', () async {
      final session = db.auth.currentSession;
      expect(db.auth, isNotNull);
      print('✅ Auth: endpoint reachable (session: ${session != null ? 'active' : 'none'})');
    });
    ...
    test('15. Realtime subscriptions are available', () async {
      ...
      print('✅ Realtime: subscriptions work');
    });
  });
}
